...
export default function HotspotInfoPopup({ hotspot, setSelectedHotspot, renderer, camera }) {
  ...
  useEffect(() => {
    ...
    const updatePopupPosition = () => {
      ...
      // Smooth the movement between previous and new position
  ...
  return (
    <div
      ref={popupRef}
      className="absolute z-10 w-64 bg-white rounded-lg shadow-lg border border-blue-100 overflow-hidden transform transition-opacity duration-200 ease-in-out"
      style={{ pointerEvents: 'auto', left: `${position.x}px`, top: `${position.y}px`, transform: 'translate3d(0,0,0)' }}
    >
      {/* Header */}
